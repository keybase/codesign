...
(function() {
  ...
  Main = (function() {
    ...
    Main.prototype.get_preset_list = function() {
      ...
      if (this.args.preset == null) {
        return exit_err("Expected comma-separated list of presets (valid values=" + (this.valid_resets().join(',')) + ")");
      } else {
        presets = this.args.preset.split(',');
        for (_i = 0, _len = presets.length; _i < _len; _i++) {
          k = presets[_i];
          if (!(__indexOf.call(this.valid_presets(), k) >= 0)) {
            exit_err("Unknown preset " + k + " (valid values=" + (this.valid_resets().join(',')) + ")");
          }
        }
        return presets;
      }
