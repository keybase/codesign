...
(function() {
  var ArgumentParser, Main, PackageJson, Summarizer, constants, from_md, fs, iced, item_type_names, to_md, __iced_k, __iced_k_noop, _ref;
  ...
  Main = (function() {
    ...
    Main.prototype.verify = function() {
      var body, err, expected, f, got, input, json_obj, summ, ___iced_passed_deferral, __iced_deferrals, __iced_k;
      __iced_k = __iced_k_noop;
      ___iced_passed_deferral = iced.findDeferral(arguments);
            ...
            if (err) {
              ...
              for (_k = 0, _len2 = _ref3.length; _k < _len2; _k++) {
                ...
                if (got.item_type !== expected.item_type) {
                  console.log("ERROR ON " + got.path + ": Expected a " + item_type_names[expected.item_type] + " and got a " + item_type_names[got.item_type]);
                }
                if (got.size !== expected.size) {
                  console.log("ERROR ON " + got.path + ": Expected size " + expected.size + " and got " + got.size);
                }
                if (got.hash !== expected.hash) {
                  console.log("ERROR ON " + got.path + ": Expected hash " + expected.hash.slice(0, 16) + "... and got " + got.hash.slice(0, 16) + "...");
                }
                if (got.link !== expected.link) {
                  console.log("ERROR ON " + got.path + ": Expected link to '" + expected.link + "' and got link to '" + got.link + "'");
                }
                if (got.exec && !expected.exec) {
                  console.log("ERROR ON " + got.path + ": Expected NO user exec privileges, but got them");
                }
                if (!got.exec && expected.exec) {
                  console.log("ERROR ON " + got.path + ": Expected user exec privileges but didn't get them");
                }
              }
