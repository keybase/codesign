...
_break()
    ...
    Summarizer.prototype.compare_to_json_obj = function(obj, cb) {
      ...
      for (p1 in exp_by_path) {
        ...
        } else if ((expected.item_type === item_types.SYMLINK) && (got.item_type === item_types.FILE) && (expected.hash.hash === got.link_hash)) {
          status = vc.ALT_SYMLINK_MATCH;
        } else if ((expected.item_type === item_types.FILE) && (got.item_type === item_types.SYMLINK) && (expected.hash.hash === got.link_hash)) {
          status = vc.ALT_SYMLINK_MATCH;
