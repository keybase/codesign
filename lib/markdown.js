...
(function() {
  var HEADINGS, SPACER, TABLIFY_OPTS, constants, files_from_pretty_format, format_signature, hash_from_str, hash_to_str, item_types, max_depth, parse_signatures, path, pretty_format_files, tablify, utils;
  ...
  exports.from_md = function(str) {
    ...
    rxx = /^\s*\#\#\#\#\sVerify\s*```([^`]*)```\s*\#\#\#\#\sPresets\s*```([^`]*)```\s*\#\#\#\#\sIgnore\s*```([^`]*)```\s*\<\!--[\s]*summarize[\s]*version[\s]*=[\s]*([0-9a-z\.]*)[\s]*-->\s*\<\!--\sBEGIN\sSIGNATURES\s--\>\s*([^\<]*)\s*\<\!--\sEND\sSIGNATURES\s--\>\s*.*\s*$/;
    match = rxx.exec(str);
    if (match != null) {
