...
(function() {
  ...
  exports.from_md = function(str) {
    ...
    rxx = /^\s*([^\<]*)\s*\<\!--\sEND\sSIGNATURES\s--\>\s*\#\#\#\sBegin\ssigned\sportion\s*\#\#\#\#\sExpect\s*```([^`]*)```\s*\#\#\#\#\sIgnore\s*```([^`]*)```\s*\#\#\#\#\sPresets\s*```([^`]*)```\s*\<\!--[\s]*summarize[\s]*version[\s]*=[\s]*([0-9a-z\.]*)[\s]*-->\s*\#\#\#\sEnd\ssigned\sportion\s*[\s\S]*\s*$/;
    match = rxx.exec(str);
    if (match != null) {
      signatures = match[1];
